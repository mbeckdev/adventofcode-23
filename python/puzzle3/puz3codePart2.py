...
realPathStringsList = ["python\\puzzle", puzzleNo, "\\p", puzzleNo, "part", "1", "input.txt"]
...
for index, game in enumerate(allGames):
    ...
    # if (game.passesCheck): 
        # ans += game.gameIdfind
    # get max of all sets - rmax gmax bmax
    # find power of a game from max max max
    # add up powers
    maxR = 0
    maxG = 0
    maxB = 0
    for set in game.setArray:
        # set.display_blocks()
        if (set.redblocks > maxR): maxR = set.redblocks
        if (set.greenblocks > maxG): maxG = set.greenblocks
        if (set.blueblocks > maxB): maxB = set.blueblocks
    # for index, set in enumerate(game.setArray):
    # print(f'maxR={maxR} maxG={maxG} maxB={maxB}')
